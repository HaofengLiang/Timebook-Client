...
export default function Calendar() {
  ...
  const { status: eventActionStatus, error: eventActionError } = useSelector(
    (state) => state.events
  );
  ...
  return (
    <Fragment>
      ...
      >
        <Alert onClose={resetActionStatusHandler} severity="error">
          {eventActionError}
        </Alert>
      </Snackbar>
      <Backdrop
        open={eventActionStatus === 'loading'}
        sx={{
          color: '#fff',
